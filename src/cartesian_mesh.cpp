...
CartesianHMeshLocalMapperT::CartesianHMeshLocalMapperT(
  ...
  for (int cz = starts[2]; cz < ends[2]; cz++) {
    ...
    for (int cy = starts[1]; cy < ends[1]; cy++) {
      ...
      for (int cx = starts[0]; cx < ends[0]; cx++) {
        const int cxs = (cx + stencil_width * cell_counts[0]) % cell_counts[0];
        index_mesh[0] = cxs;
        ...
        coord[1] =
            (this->ndim > 1) ? this->dh_cell_lookups[1]->h_buffer.ptr[cys] : 0;
        coord[2] =
            (this->ndim > 2) ? this->dh_cell_lookups[2]->h_buffer.ptr[czs] : 0;
...
void CartesianHMeshLocalMapperT::map(ParticleGroup &particle_group,
                                     const int map_cell) {
...
void CartesianHMeshLocalMapperT::map_cells(ParticleGroup &particle_group,
                                           const int map_cell) {
