#include <neso_particles/communication/communication_edges_counter.hpp>

namespace NESO::Particles {

CommunicationEdgesCounter::CommunicationEdgesCounter(MPI_Comm comm)
    : comm(comm) {
...
void CommunicationEdgesCounter::reset() {
  recv_win_data[0] = 0;
  MPICHK(MPI_Ibarrier(this->comm, &this->mpi_request));
}

void CommunicationEdgesCounter::init_count(std::vector<int> &ranks) {
...
int CommunicationEdgesCounter::get_count() {
  MPICHK(MPI_Wait(&this->mpi_request, MPI_STATUS_IGNORE));
  return this->recv_win_data[0];
}

void CommunicationEdgesCounter::exchange_send_recv_counts(
    std::vector<int> &recv_ranks, std::vector<std::int64_t> &recv_data,
    std::vector<int> &send_ranks, std::vector<std::int64_t> &send_data,
    const bool ordered) {
...
void CommunicationEdgesCounter::exchange_send_recv_data(
    std::vector<int> &recv_ranks, std::vector<std::int64_t> &recv_counts,
    std::vector<void *> &recv_data, std::vector<int> &send_ranks,
    std::vector<std::int64_t> &send_counts, std::vector<void *> &send_data) {
  const int num_recv_ranks = recv_ranks.size();
  const int num_send_ranks = send_ranks.size();
  std::vector<MPI_Request> send_requests(num_send_ranks);
  std::vector<MPI_Request> recv_requests(num_recv_ranks);
...
void CommunicationEdgesCounter::get_remote_ranks(
    std::vector<int> &recv_ranks, std::vector<std::int64_t> &recv_data,
    std::vector<int> &send_ranks, std::vector<std::int64_t> &send_data) {
  this->reset();
  ...
  this->exchange_send_recv_counts(recv_ranks, recv_data, send_ranks, send_data);
}

void CommunicationEdgesCounter::free() {
