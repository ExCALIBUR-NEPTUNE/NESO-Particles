...
INT VTKMeshHierarchyCellsWriter::get_index(std::tuple<INT, INT, INT> vert) {
...
VTKMeshHierarchyCellsWriter::to_standard_tuple(const INT linear_index) {
  ...
  for (int dimx = 0; dimx < ndim; dimx++) {
    stuple[dimx] = tuple_cell[dimx] * num_cells_fine + tuple_cell[dimx + ndim];
  }
  return {stuple[0], stuple[1], stuple[2]};
}

std::vector<std::tuple<INT, INT, INT>>
VTKMeshHierarchyCellsWriter::get_coarse_tuples() {
...
VTKMeshHierarchyCellsWriter::get_lines(std::tuple<INT, INT, INT> base) {
  std::vector<std::pair<std::tuple<INT, INT, INT>, std::tuple<INT, INT, INT>>>
      lines;
...
void VTKMeshHierarchyCellsWriter::write_inner(std::string filename,
                                              const bool fine) {
...
void VTKMeshHierarchyCellsWriter::push_back(const INT linear_index) {
  this->cells.push_back(linear_index);
}

void VTKMeshHierarchyCellsWriter::write(std::string filename) {
  write_inner(filename, true);
}

void VTKMeshHierarchyCellsWriter::write_all_fine(std::string filename) {
...
void VTKMeshHierarchyCellsWriter::write_all_coarse(std::string filename) {
  const int rank = this->mesh_hierarchy->comm_pair.rank_parent;
  if (rank == 0) {
