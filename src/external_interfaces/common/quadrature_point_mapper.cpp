...
#include <neso_particles/external_interfaces/common/quadrature_point_mapper.hpp>

namespace NESO::Particles::ExternalCommon {

void QuadraturePointMapper::make_contribs_buffers(int ncomp_local,
                                                  int ncomp_remote) {
...
QuadraturePointMapper::QuadraturePointMapper(SYCLTargetSharedPtr sycl_target,
                                             DomainSharedPtr domain)
    : ndim(domain->mesh->get_ndim()), rank(sycl_target->comm_pair.rank_parent),
...
void QuadraturePointMapper::add_point(const REAL *point) {
  Point p;
...
void QuadraturePointMapper::add_points_finalise() {
  ...
  particle_loop(
      ...
      Access::write(la_remote_indices), Access::add(la_remote_indices_counter),
      Access::write(la_tmp_index))
      ->execute();
  ...
  std::exclusive_scan(adding_indices_count.begin(), adding_indices_count.end(),
                      adding_write_starts.begin(), 0);
  ...
  particle_loop(
      ...
      [=](auto INDEX, auto ADDING_RANK_INDEX, auto RANK_STARTS, auto TMP_INDEX,
          auto REMOTE_INDICES) {
        // Was this point added by a remote rank?
      ...
      Access::read(ParticleLoopIndex{}),
      Access::write(Sym<INT>("ADDING_RANK_INDEX")), Access::add(la_rank_starts),
      Access::read(la_tmp_index), Access::write(la_remote_indices))
      ->execute();
  ...
  this->cec->exchange_send_recv_data(this->adding_ranks, this->adding_num_bytes,
                                     this->adding_ptrs, this->owning_ranks,
                                     this->owning_num_bytes, this->owning_ptrs);
...
Sym<REAL> QuadraturePointMapper::get_sym(const int ncomp) {
...
void QuadraturePointMapper::get(const int ncomp, std::vector<REAL> &output) {
  ...
  for (int ix = 0; ix < num_entries; ix++) {
    ...
    for (int cx = 0; cx < ncomp; cx++) {
      output.at(dst_index * ncomp + cx) = this->recv_buffer.at(ix * ncomp + cx);
    }
  }
}

void QuadraturePointMapper::set(const int ncomp, std::vector<REAL> &input) {
  auto sym = this->get_sym(ncomp);
  const int ncomp_local = this->npoint_local * ncomp;
...
void QuadraturePointMapper::free() {
...
bool QuadraturePointMapper::points_added() {
  return this->internal_points_added;
}

void QuadraturePointMapper::write_to_disk(std::string filename) {
