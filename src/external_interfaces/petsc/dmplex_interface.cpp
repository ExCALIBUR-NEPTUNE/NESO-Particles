...
void DMPlexInterface::create_mesh_hierarchy() {
  ...
  this->subdivision_order =
      std::max(0, matching_subdivision_order + this->subdivision_order_offset);
...
void DMPlexInterface::get_unowned_cells(
...
void DMPlexInterface::claim_mesh_hierarchy_cells(
  ...
  for (int cellx = 0; cellx < num_cells; cellx++) {
    ...
    ExternalCommon::bounding_box_claim(
        cellx, bounding_box, this->mesh_hierarchy, local_claim, mh_element_map);
  }
...
void DMPlexInterface::create_halos(ExternalCommon::MHGeomMap &mh_element_map) {
...
DMPlexInterface::DMPlexInterface(DM dm, const int subdivision_order_offset,
                                 MPI_Comm comm)
...
void DMPlexInterface::get_point_in_subdomain(double *point) {
...
std::vector<int> &DMPlexInterface::get_local_communication_neighbours() {
  return this->neighbour_ranks;
}
MPI_Comm DMPlexInterface::get_comm() { return this->comm; };
int DMPlexInterface::get_ndim() { return this->ndim; };
std::vector<int> &DMPlexInterface::get_dims() {
  return this->mesh_hierarchy->dims;
};
int DMPlexInterface::get_subdivision_order() {
  return this->subdivision_order;
};
int DMPlexInterface::get_cell_count() { return this->cell_count; };
double DMPlexInterface::get_cell_width_coarse() {
  return this->mesh_hierarchy->cell_width_coarse;
};
double DMPlexInterface::get_cell_width_fine() {
  return this->mesh_hierarchy->cell_width_fine;
};
double DMPlexInterface::get_inverse_cell_width_coarse() {
  return this->mesh_hierarchy->inverse_cell_width_coarse;
};
double DMPlexInterface::get_inverse_cell_width_fine() {
  return this->mesh_hierarchy->inverse_cell_width_fine;
};
int DMPlexInterface::get_ncells_coarse() {
  return this->mesh_hierarchy->ncells_coarse;
};
int DMPlexInterface::get_ncells_fine() {
  return this->mesh_hierarchy->ncells_fine;
};
std::shared_ptr<MeshHierarchy> DMPlexInterface::get_mesh_hierarchy() {
  return this->mesh_hierarchy;
}

bool DMPlexInterface::validate_halos(const bool fatal) {
  ...
  for (int ix = 0; ix < num_local_points; ix++) {
    max_global_point = std::max(
        max_global_point, this->dmh->get_point_global_index(ix + point_start));
  }
  int num_points;
