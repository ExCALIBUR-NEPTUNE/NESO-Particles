...
#include <neso_particles/external_interfaces/petsc/particle_cell_mapping/dmplex_host_mapper.hpp>

namespace NESO::Particles::PetscInterface {

DMPlexHostMapper::DMPlexHostMapper(SYCLTargetSharedPtr sycl_target,
...
void DMPlexHostMapper::map(ParticleGroup &particle_group, const int map_cell) {
  ...
  const PetscInt local_ncomp = static_cast<PetscInt>(npart_local) * ndim_coord;
  ...
  interlace_loop->wait();
  sycl_target->queue
      .memcpy(v_ptr, k_interlaced_positions, local_ncomp * sizeof(PetscScalar))
      .wait_and_throw();
  ...
  if (this->dmplex_interface->dmh_halo) {
    ...
    cell_sf = nullptr;
    PETSCCHK(DMLocatePoints(dm_halo, v_halo, DM_POINTLOCATION_NONE, &cell_sf));
    PETSCCHK(PetscSFGetGraph(cell_sf, NULL, &n_found, &found, &cells));
    for (int qx = 0; qx < npart_not_found; qx++) {
      const int px = not_local_found.at(qx);
      ...
      if (cell_found) {
        auto rank_cell = dmplex_interface->map_local_lid_remote_lid.at(px_cell);
