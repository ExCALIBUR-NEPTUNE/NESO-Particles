...
VTKHDF::VTKHDF(std::string filename, MPI_Comm comm, std::string dataset_type)
  ...
  H5CHK(this->file_id = H5Fcreate(filename.c_str(), H5F_ACC_TRUNC, H5P_DEFAULT,
                                  this->plist_id));
...
void VTKHDF::write(std::vector<UnstructuredCell> &data) {
  ...
  int counts_global[4];
  MPICHK(
      MPI_Exscan(counts_local, offsets_array, 4, MPI_INT, MPI_SUM, this->comm));
  ...
  this->write_dataset_2d(this->root, npoint_global, point_offset, npoint_local,
                         3, H5T_IEEE_F64LE, H5T_NATIVE_DOUBLE, "Points",
                         points);
  ...
  this->write_dataset(this->root, noffset_global, offset_offset, H5T_STD_I64LE,
                      H5T_NATIVE_INT, "Offsets", offsets);
