...
void CollisionCellPartition::get_num_unmasked_particles(
    const INT species_id, NDLocalArraySharedPtr<int, 2> &num_particles) {
  ...
  if (num_particles == nullptr) {
    num_particles = std::make_shared<NDLocalArray<int, 2>>(this->sycl_target,
                                                           required_size);
  } else {
    NESOASSERT(num_particles->index == required_size,
               "Miss-match of num_particles size and required size.");
      ...
      .parallel_for(iteration_set,
                    [=](sycl::item<2> ix) {
                      ...
                      const auto num_particles =
                          k_map.get_num_particles_cell_species(
                              cell_mesh, cell_collision, linear_species_id);
                      k_cell_counts[ix.get_linear_id()] =
