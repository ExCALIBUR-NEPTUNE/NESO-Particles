#include <neso_particles/algorithms/dsmc/pair_sampling_ntc.hpp>

namespace NESO::Particles::DSMC {

PairSamplingNTC::PairSamplingNTC(
...
void PairSamplingNTC::sample(ParticleSubGroupSharedPtr sub_group_a,
  ...
  for (int cx = 0; cx < cell_count; cx++) {
    ...
    max_num_blocks = std::max(
        ...
        static_cast<int>(get_next_multiple(required_size, block_size)) /
            block_size);
    this->h_pair_counts_es[cx] = required_size;
    ...
    cgh.parallel_for(nd_iteration_set, [=](sycl::nd_item<2> idx) {
      ...
      for (int num_sampled_pairs = 0; num_sampled_pairs < npairs;
        ...
        if ((num_sampled_pairs + local_id) < npairs) {
          ...
          const int i2 = i1;
          const int j2 = a_is_b ? (i1 + j1) % npart_cell_a : j1;
        ...
        if (wave >= 0) {
          k_pair_list[ox + pair_index] = i3;
          k_pair_list[k_pair_count * 1 + ox + pair_index] = j3;
          k_pair_list[k_pair_count * 2 + ox + pair_index] = wave;
        } else {
          k_pair_list[k_pair_count * 2 + ox + pair_index] = -1;
        }
...
CellwisePairListBlockDevice PairSamplingNTC::get_pair_list() {
