...
private:
  ...
  inline void execute_new() {
    NESOASSERT(this->style_new,
               "Expected to be initialised with new style looping.");
    NESOASSERT(this->pbc_loop != nullptr, "ParticleLoop not defined.");
    this->pbc_loop->execute();
  }
...
public:
  ...
  CartesianPeriodic(SYCLTargetSharedPtr sycl_target,
      ...
      : sycl_target(sycl_target), mesh(mesh), position_dat(position_dat),
        d_extents(sycl_target, 3), style_old(true), style_new(false) {
  ...
  CartesianPeriodic(std::shared_ptr<CartesianHMesh> mesh,
      ...
      : mesh(mesh), sycl_target(particle_group->sycl_target),
        d_extents(particle_group->sycl_target, 3), style_old(false),
        style_new(true) {
  ...
  inline void execute() {
    ...
    if (style_old) {
      ...
      NESOASSERT(style_new, "Expected new stlye looping");
      this->execute_new();
    }
