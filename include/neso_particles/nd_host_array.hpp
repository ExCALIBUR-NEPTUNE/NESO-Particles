...
#include "containers/rng/rng_generation_function.hpp"
...
public:
  ...
  fill(std::shared_ptr<RNGGenerationFunction<T>> rng_generation_function) {
    ...
    auto d_buffer =
        get_resource<BufferDevice<T>, ResourceStackInterfaceBufferDevice<T>>(
            ...
            ResourceStackKeyBufferDevice<T>{}, this->sycl_target);

    const auto s = this->index.size();
    d_buffer->realloc_no_copy(s);

    rng_generation_function->draw_random_samples(this->sycl_target,
                                                 d_buffer->ptr, s, 8192);
    this->sycl_target->queue.memcpy(this->ptr(), d_buffer->ptr, s * sizeof(T))
        .wait_and_throw();

    restore_resource(this->sycl_target->resource_stack_map,
                     ResourceStackKeyBufferDevice<T>{}, d_buffer);
  }
   ...
   * fastest.
   *
   * @returns Vector of linearised entries.
   */
