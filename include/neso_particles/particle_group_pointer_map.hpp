...
public:
  ...
  ParticleGroupPointerMap(
    ...
    this->dh_dat_ptr_real = std::make_shared<BufferDeviceHost<REAL ***>>(
        this->sycl_target, ndat_real);
    this->dh_dat_ptr_int = std::make_shared<BufferDeviceHost<INT ***>>(
        this->sycl_target, ndat_int);
   ...
   */
  inline ParticleGroupPointerMapDeviceConst get_const() {
    if (!this->valid) {
      ...
      for (auto [sym, dat] : *this->particle_dats_real) {
        ...
        this->dh_dat_ptr_const_real->h_buffer.ptr[index] =
            dat->impl_get_const();
      ...
      this->d_cache_const.d_ptr_real =
          this->dh_dat_ptr_const_real->d_buffer.ptr;
