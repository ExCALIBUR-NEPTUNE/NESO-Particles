...
TEST(ParticleLoopReduction, benchmark) {
  const int N = 1000000;
  auto B = particle_loop_common(N);
  auto domain = B->domain;
  auto mesh = domain->mesh;
  auto sycl_target = B->sycl_target;
  const int cell_count = mesh->get_cell_count();
  ...
  auto lambda_do_run = [&](auto A) {
    const auto N = A->get_npart_local();
    ...
    auto loop5 = particle_loop_reduction(
        ...
        Access::write(SymVector<REAL>(get_particle_group(A), syms)));
    loop5->execute();
  ...
  lambda_do_run(B);
  lambda_do_run(static_particle_sub_group(
