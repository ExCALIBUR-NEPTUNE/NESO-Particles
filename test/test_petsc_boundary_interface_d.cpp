...
TEST(PETScBoundary2D, reflection_truncated) {
  ...
  auto b2d = std::make_shared<TestBoundaryInteraction2D>(sycl_target, mesh,
                                                         boundary_groups);
  auto reflection = std::make_shared<BoundaryReflection>(2, 1.0e-10);
...
void gmsh_reflection_test_wrapper(DM dm,
                                  std::vector<PetscInt> boundary_labels) {
  ...
  std::map<PetscInt, std::vector<PetscInt>> boundary_groups;
  boundary_groups[1] = boundary_labels;
...
TEST(PETScBoundary2D, reflection_advection) {
  ...
  GET_TEST_RESOURCE(mesh_file, "gmsh/mesh_ring.msh");
  if (mesh_file.size()) {
    ...
    gmsh_reflection_test_wrapper(dm, {1, 2, 3, 4});
  }
...
TEST(PETScBoundary2D, reflection_advection_hypnotoad) {
  ...
  GET_TEST_RESOURCE(mesh_file,
                    "gmsh/mesh_hypnotoad_anticlockwise_vertices.msh");
  ...
  if (mesh_file.size()) {
    ...
    gmsh_reflection_test_wrapper(dm, {100});
  }
