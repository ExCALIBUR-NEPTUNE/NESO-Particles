...
TEST(BoundaryReflection, 2d) {
  ...
  A->add_particle_dat(Sym<REAL>("VV"), 2);
  ...
  particle_loop(
      ...
      Access::read(Sym<REAL>("V")), Access::write(Sym<REAL>("VV")))
      ->execute();
  ...
  particle_loop(
      groups.at(0),
      [=](auto V, auto VV, auto NORMAL) {
        const REAL v[2] = {V.at(0), V.at(1)};
        const REAL vv[2] = {VV.at(0), VV.at(1)};
        const REAL n[2] = {NORMAL.at_ephemeral(0), NORMAL.at_ephemeral(1)};

        const REAL vdotn = Kernel::dot_product_2d(v, n);
        const REAL vvdotn = Kernel::dot_product_2d(vv, n);
        NESO_KERNEL_ASSERT(Kernel::abs(vdotn + vvdotn) < 1.0e-14, k_ep);

        const REAL vperpn[2] = {v[0] - vdotn * n[0], v[1] - vdotn * n[1]};
        const REAL vvperpn[2] = {vv[0] - vvdotn * n[0], vv[1] - vvdotn * n[1]};

        NESO_KERNEL_ASSERT(Kernel::abs(vperpn[0] - vvperpn[0]) < 1.0e-14, k_ep);
        NESO_KERNEL_ASSERT(Kernel::abs(vperpn[1] - vvperpn[1]) < 1.0e-14, k_ep);
      },
...
TEST(BoundaryReflection, 3d) {
  ...
  A->add_particle_dat(Sym<REAL>("VV"), 3);
  ...
  particle_loop(
      groups.at(0),
      [=](auto V, auto VV, auto NORMAL) {
        const REAL v[3] = {V.at(0), V.at(1), V.at(2)};
        const REAL vv[3] = {VV.at(0), VV.at(1), VV.at(2)};
        const REAL n[3] = {NORMAL.at_ephemeral(0), NORMAL.at_ephemeral(1),
                           NORMAL.at_ephemeral(2)};

        const REAL vdotn = Kernel::dot_product_3d(v, n);
        const REAL vvdotn = Kernel::dot_product_3d(vv, n);
        NESO_KERNEL_ASSERT(Kernel::abs(vdotn + vvdotn) < 1.0e-14, k_ep);

        const REAL vperpn[3] = {v[0] - vdotn * n[0], v[1] - vdotn * n[1],
                                v[2] - vdotn * n[2]};
        const REAL vvperpn[3] = {vv[0] - vvdotn * n[0], vv[1] - vvdotn * n[1],
                                 vv[2] - vvdotn * n[2]};

        NESO_KERNEL_ASSERT(Kernel::abs(vperpn[0] - vvperpn[0]) < 1.0e-14, k_ep);
        NESO_KERNEL_ASSERT(Kernel::abs(vperpn[1] - vvperpn[1]) < 1.0e-14, k_ep);
        NESO_KERNEL_ASSERT(Kernel::abs(vperpn[2] - vvperpn[2]) < 1.0e-14, k_ep);
      },
      Access::read(Sym<REAL>("V")), Access::read(Sym<REAL>("VV")),
      Access::read(BoundaryInteractionSpecification::intersection_normal))
      ->execute();
